...
/**
 * AR-1a — Customer Master
 * (docs/finance-work-item-ar-1a-customer-master-ar-foundation-proposal.md
 * §3, §5, §7). Same shape as suppliers.e2e-spec.ts: proves RBAC is
 ...
 */
describe("Customers (e2e) — RBAC, tenant + legal-entity isolation, audit", () => {
  ...
  let accountA1Id: string; // active REVENUE account, entity A1 — for defaultRevenueAccountId
  let accountA2Id: string; // active REVENUE account, entity A2 — proves cross-entity rejection
  ...
  beforeAll(async () => {
    ...
    const [tenantA] = await db
      .insert(tenants)
      .values({ slug: `ar-e2e-a-${suffix}`, name: "AR E2E Tenant A" })
      .returning();
    const [tenantB] = await db
      .insert(tenants)
      .values({ slug: `ar-e2e-b-${suffix}`, name: "AR E2E Tenant B" })
      .returning();
  ...
  describe("RBAC — enforced server-side", () => {
    ...
    it("allows finance.poster to list (200) — customers must be readable by the role that will create invoices against them", async () => {
    ...
    it("rejects finance.poster attempting to create (403) — only finance.admin manages customer master data", async () => {
      const token = tokenFor(tenantAId, legalEntityA1Id, ["finance.poster"]);
      await request(app.getHttpServer())
        .post("/v1/finance/customers")
        .set("Authorization", `Bearer ${token}`)
  ...
  describe("validation", () => {
    ...
    it("rejects a nonexistent defaultRevenueAccountId (400)", async () => {
      const token = tokenFor(tenantAId, legalEntityA1Id, ["finance.admin"]);
      await request(app.getHttpServer())
        .post("/v1/finance/customers")
        .set("Authorization", `Bearer ${token}`)
        .send({
          ...
          defaultRevenueAccountId: randomUUID(),
        })
        .expect(400);
    });

    it("rejects a defaultRevenueAccountId belonging to a different legal entity (400)", async () => {
      const token = tokenFor(tenantAId, legalEntityA1Id, ["finance.admin"]);
      await request(app.getHttpServer())
        .post("/v1/finance/customers")
        .set("Authorization", `Bearer ${token}`)
        .send({
          ...
          defaultRevenueAccountId: accountA2Id, // belongs to entity A2, caller is A1
        })
        .expect(400);
    });

    it("accepts a defaultRevenueAccountId belonging to the caller's own legal entity (201)", async () => {
      const token = tokenFor(tenantAId, legalEntityA1Id, ["finance.admin"]);
      const res = await request(app.getHttpServer())
        .post("/v1/finance/customers")
        .set("Authorization", `Bearer ${token}`)
        .send({
          ...
          defaultRevenueAccountId: accountA1Id,
          paymentTermsDays: 30,
          taxRegistrationNo: "AE-VAT-999",
        })
        .expect(201);
      expect(res.body.data.defaultRevenueAccountId).toBe(accountA1Id);
    ...
    it("allows the same code under two different legal entities of the same tenant (201 both)", async () => {
      ...
      await request(app.getHttpServer())
        ...
        .send({ code, name: "Entity 1 customer" })
        .expect(201);
      await request(app.getHttpServer())
        .post("/v1/finance/customers")
        .set("Authorization", `Bearer ${tokenA2}`)
        .send({ code, name: "Entity 2 customer" })
        .expect(201);
    });
  });
  ...
  describe("read / update / list", () => {
    ...
    it("retrieves the created customer by id", async () => {
      const token = tokenFor(tenantAId, legalEntityA1Id, ["finance.viewer"]);
      const res = await request(app.getHttpServer())
        .get(`/v1/finance/customers/${customerId}`)
        .set("Authorization", `Bearer ${token}`)
        .expect(200);
    ...
    it("returns 404 for a nonexistent customer id", async () => {
      const token = tokenFor(tenantAId, legalEntityA1Id, ["finance.viewer"]);
      await request(app.getHttpServer())
        .get(`/v1/finance/customers/${randomUUID()}`)
        .set("Authorization", `Bearer ${token}`)
        .expect(404);
    });

    it("updates the customer's editable fields", async () => {
      const token = tokenFor(tenantAId, legalEntityA1Id, ["finance.admin"]);
      const res = await request(app.getHttpServer())
        .patch(`/v1/finance/customers/${customerId}`)
        .set("Authorization", `Bearer ${token}`)
    ...
    it("rejects updating to a cross-entity defaultRevenueAccountId (400)", async () => {
      const token = tokenFor(tenantAId, legalEntityA1Id, ["finance.admin"]);
      await request(app.getHttpServer())
        .patch(`/v1/finance/customers/${customerId}`)
        .set("Authorization", `Bearer ${token}`)
        .send({ defaultRevenueAccountId: accountA2Id })
        .expect(400);
    });

    it("lists exclude inactive customers by default, include with includeInactive=true", async () => {
      const adminToken = tokenFor(tenantAId, legalEntityA1Id, [
        "finance.admin",
      ]);
      await request(app.getHttpServer())
        .patch(`/v1/finance/customers/${customerId}/deactivate`)
      ...
      const fullList = await request(app.getHttpServer())
        .get("/v1/finance/customers?includeInactive=true")
        .set("Authorization", `Bearer ${adminToken}`)
        .expect(200);
    ...
    it("reactivate brings the customer back into the default list", async () => {
      const adminToken = tokenFor(tenantAId, legalEntityA1Id, [
        "finance.admin",
      ]);
      const res = await request(app.getHttpServer())
        .patch(`/v1/finance/customers/${customerId}/reactivate`)
        .set("Authorization", `Bearer ${adminToken}`)
        .expect(200);
      expect(res.body.data.isActive).toBe(true);
  ...
  describe("cross-tenant isolation", () => {
    ...
    beforeAll(async () => {
      ...
      const resA = await request(app.getHttpServer())
        ...
        .send({ code: `ISO-A-${suffix}`, name: "Tenant A Customer" })
        .expect(201);
      customerAId = resA.body.data.id;
      ...
      const resB = await request(app.getHttpServer())
        ...
        .send({ code: `ISO-B-${suffix}`, name: "Tenant B Customer" })
        .expect(201);
      customerBId = resB.body.data.id;
    });

    it("tenant A lists: sees its own customer, not tenant B's", async () => {
      const res = await request(app.getHttpServer())
        .get("/v1/finance/customers")
        .set(
          "Authorization",
    ...
    it("tenant A cannot directly read tenant B's customer by id (404)", async () => {
      await request(app.getHttpServer())
        .get(`/v1/finance/customers/${customerBId}`)
        .set(
          "Authorization",
          `Bearer ${tokenFor(tenantAId, legalEntityA1Id, ["finance.viewer"])}`,
        )
    ...
    it("tenant A cannot write (deactivate) tenant B's customer — RLS blocks it, and the attempt has no effect", async () => {
      await request(app.getHttpServer())
        .patch(`/v1/finance/customers/${customerBId}/deactivate`)
        .set(
          "Authorization",
          `Bearer ${tokenFor(tenantAId, legalEntityA1Id, ["finance.admin"])}`,
        )
  ...
  describe("cross-legal-entity isolation within the same tenant", () => {
    ...
    beforeAll(async () => {
      ...
      const resA1 = await request(app.getHttpServer())
        ...
        .send({ code: `LE-ISO-${suffix}`, name: "Entity 1 Customer" })
        .expect(201);
      customerA1Id = resA1.body.data.id;
      ...
      const resA2 = await request(app.getHttpServer())
        ...
        .send({ code: `LE-ISO-${suffix}`, name: "Entity 2 Customer" })
        .expect(201);
      customerA2Id = resA2.body.data.id;
    });

    it("entity 1 lists: sees its own customer, not entity 2's — same tenant, same RLS session var", async () => {
      const res = await request(app.getHttpServer())
        .get("/v1/finance/customers")
        .set(
          "Authorization",
        ...
        .expect(200);
      const ids = res.body.data.map((s: { id: string }) => s.id);
      expect(ids).toContain(customerA1Id);
      expect(ids).not.toContain(customerA2Id);
    });

    it("entity 1 cannot directly read entity 2's customer by id (404), even within the same tenant", async () => {
      await request(app.getHttpServer())
        .get(`/v1/finance/customers/${customerA2Id}`)
        .set(
          "Authorization",
          `Bearer ${tokenFor(tenantAId, legalEntityA1Id, ["finance.viewer"])}`,
        )
    ...
    it("entity 1 cannot deactivate entity 2's customer, and the attempt has no effect", async () => {
      await request(app.getHttpServer())
        .patch(`/v1/finance/customers/${customerA2Id}/deactivate`)
        .set(
          "Authorization",
          `Bearer ${tokenFor(tenantAId, legalEntityA1Id, ["finance.admin"])}`,
        )
  ...
  describe("audit trail — written per-action and genuinely append-only", () => {
    it("records a CREATE entry scoped to the acting tenant and legal entity", async () => {
      ...
      const res = await request(app.getHttpServer())
        .post("/v1/finance/customers")
        .set("Authorization", `Bearer ${token}`)
        .send({ code, name: "Audited customer" })
        .expect(201);
    ...
    it("records DEACTIVATE and REACTIVATE entries with before/after state", async () => {
      ...
      const created = await request(app.getHttpServer())
        .post("/v1/finance/customers")
        .set("Authorization", `Bearer ${token}`)
        .send({ code: `AUDIT-STATUS-${Date.now()}`, name: "Status customer" })
        .expect(201);
      const customerId = created.body.data.id;
