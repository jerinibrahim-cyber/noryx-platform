...
**Current baseline:** `main` — Tax/VAT Phase 5 (Per-Tax-Code GL Account Mapping) is implemented, verified (typecheck/lint clean, 610/610 unit tests, 891/891 e2e tests, 137/137 RBAC-matrix assertions, migration `0020_tax_vat_phase_5_gl_account_mapping.sql` applied and directly verified against both `noryx` and `noryx_test`), and committed to local `main`. See `docs/finance-work-item-tax-vat-phase-5-completion-report.md` for the full verification record, including exact commit SHA(s) and GitHub push status.

**Completed immediately before Tax/VAT Phase 5:** Tax/VAT Phase 4 (VAT Position Report, `ba607b8`/`8ccdec5`); before that Tax/VAT Phase 3 (AR Tax Calculation, `ad71a50`/`263354b`); before that Tax/VAT Phase 2 (AP Tax Calculation, `ae4b073`/`6229bc6`); before that Tax/VAT Phase 1 (Tax Configuration Foundation, `dd6d135`); before that, Banking 1A–1E, Banking Reconciliation, and Scheduled Reversal are treated as complete per the governing Finance baseline.
...
- [x] Phase 7 — VAT Position Detail / Source-Document Drill-Down (implemented, at CTO_REVIEW on branch `feat/tax-vat-phase-7-vat-position-detail-drill-down` — see `docs/work-items/tax-vat-phase-7-vat-position-detail-drill-down/COMPLETION_REPORT.md`): a new, additive, read-only `GET /tax-reports/vat-position-detail` returns one row per persisted tax-bearing source line contributing to the VAT Position Report's `netTaxMinor`, across all five source types (the four Phase 2/3 AP/AR line tables plus Phase 6's manually-tax-classified `journal_lines`), with deterministic ordering, database-side pagination, and a same-request `reconciliationTotals` array reconciling to the complete filtered result. Tax-contribution scope only (no line-grain supply-value reconciliation — deliberately not the deprioritized "Manual Journal Line Supply-Value Capture" candidate). No schema change, no migration, no change to the existing aggregate route's shape.
