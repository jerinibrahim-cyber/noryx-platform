...
**Next approved work item:** another Finance roadmap item. Not yet discovered or authorized. Manually-posted (non-AP/AR) tax journal entry coverage remains the recommended fast-follow candidate (per the Phase 5 discovery's own candidate evaluation), but is NOT selected or authorized by this update.
...
**Tax / VAT — COMPLETE for the current MVP scope (Phases 1-5); statutory/jurisdiction expansion deferred, see "Later" below.**
...
- [ ] Later — statutory VAT filing formats, reverse charge, multi-jurisdiction expansion, tax-inclusive pricing, and coverage of manually-posted (non-AP/AR) tax journal entries — all deferred items from the approved architecture, restated in the Phase 4 discovery's own risk section (§10) and re-evaluated (not selected) in the Phase 5 discovery's own candidate comparison.
