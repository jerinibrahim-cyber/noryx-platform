...
**Current baseline:** `main` at `00746b36799d59db54002ab6d58ccc1120c3efe9` — Tax/VAT Phase 7 (VAT Position Detail / Source-Document Drill-Down) and repository governance/cleanup reconciliation are delivered to `main`. See `docs/work-items/tax-vat-phase-7-vat-position-detail-drill-down/COMPLETION_REPORT.md` for the Phase 7 verification record and `docs/engineering/REPOSITORY_CLEANUP_REPORT_2026-09.md` for the cleanup record.

**Completed immediately before Tax/VAT Phase 7:** Tax/VAT Phase 6 (Manual Journal Tax Coverage, `2bcb131`); before that Budgeting Phase 1 Foundation (`ac16fa0e...`); before that Tax/VAT Phase 5 (Per-Tax-Code GL Account Mapping); before that Tax/VAT Phase 4 (VAT Position Report, `ba607b8`/`8ccdec5`); before that Tax/VAT Phase 3 (AR Tax Calculation, `ad71a50`/`263354b`); before that Tax/VAT Phase 2 (AP Tax Calculation, `ae4b073`/`6229bc6`); before that Tax/VAT Phase 1 (Tax Configuration Foundation, `dd6d135`); before that, Banking 1A–1E, Banking Reconciliation, and Scheduled Reversal are treated as complete per the governing Finance baseline.

**Current work item:** None. Repository cleanup, governance integration, and Tax/VAT Phases 1–7 are delivered. Any subsequent work requires explicit CTO discovery/authorization.
...
**Tax/VAT Phase 7 — COMPLETE (DELIVERED to `main`):** VAT Position Detail / Source-Document Drill-Down — a new, additive, read-only `GET /v1/finance/tax-reports/vat-position-detail` (same `TaxReportsController`, same three finance read roles) returns one row per persisted tax-bearing source line contributing to `getVatPosition()`'s `netTaxMinor`, across all five source types (the four Phase 2/3 AP/AR line tables plus Phase 6's manually-tax-classified `journal_lines`), combined via `UNION ALL` and filtered/ordered/paginated entirely at the database layer inside the same `REPORT_TX_CONFIG` (`REPEATABLE READ`, `READ ONLY`) transaction `getVatPosition()`/`getLedger()` already use. Deterministic total ordering (`sourceDocumentDate ASC, sourceType ASC, sourceLineId ASC` — `sourceType` interposed because the five source tables are independent UUID namespaces, not one globally-unique id space), and a same-request `reconciliationTotals` array reconciling to the complete filtered result (proven, behaviorally, distinct from merely the returned page) are both frozen invariants, verified by a dedicated 14-scenario e2e suite (including a genuine concurrent-write REPEATABLE READ snapshot-isolation test, mirroring `general-ledger-concurrency.e2e-spec.ts`'s own established mechanism) plus the full pre-existing regression suite passing unmodified. Tax-contribution scope only (reconciles to `netTaxMinor`; no line-grain supply-value reconciliation — a deliberate, evidenced scope boundary, not an oversight). No schema change, no migration, no change to the existing aggregate route's shape. Delivered to `main` at `e2c52ac8ed0c42789e3f8657afe80bced6ad4bd9`, and reconciled with repository cleanup into `00746b36799d59db54002ab6d58ccc1120c3efe9`. Authoritative artifacts remain under `docs/work-items/tax-vat-phase-7-vat-position-detail-drill-down/` (`CONTRACT.md`, `ACCEPTANCE.md`, `COMPLETION_REPORT.md`).

**Next approved work item:** Not yet discovered or authorized. Any next Finance work item requires a separate, explicit CTO discovery/authorization prompt.
...
│   ├── Tax / VAT                       (COMPLETE for the current scope — Phases 1–7 all DELIVERED; statutory filing/reverse charge/multi-jurisdiction deferred, see below)
...
**Tax / VAT — COMPLETE for the current MVP scope (Phases 1-7); statutory/jurisdiction expansion deferred, see "Later" below.**
...
- [x] Phase 7 — VAT Position Detail / Source-Document Drill-Down (DELIVERED to `main` — see `docs/work-items/tax-vat-phase-7-vat-position-detail-drill-down/COMPLETION_REPORT.md`): a new, additive, read-only `GET /tax-reports/vat-position-detail` returns one row per persisted tax-bearing source line contributing to the VAT Position Report's `netTaxMinor`, across all five source types (the four Phase 2/3 AP/AR line tables plus Phase 6's manually-tax-classified `journal_lines`), with deterministic ordering, database-side pagination, and a same-request `reconciliationTotals` array reconciling to the complete filtered result. Tax-contribution scope only (no line-grain supply-value reconciliation — deliberately not the deprioritized "Manual Journal Line Supply-Value Capture" candidate). No schema change, no migration, no change to the existing aggregate route's shape.
- [ ] Later — statutory VAT filing formats, reverse charge, multi-jurisdiction expansion, tax-inclusive pricing, and line-grain supply-value reconciliation for the drill-down report — deferred items from the approved architecture, restated in the Phase 4 discovery's own risk section (§10) and Phase 7's own explicit scope decision. Manually-posted tax journal entry coverage (formerly listed here) is now Phase 6, COMPLETE, above.
...
- [x] 3.1 — Tenant/RLS Hardening — completed (`docs/finance/planning/finance-milestone-3.1-tenant-rls-hardening-proposal.md`).
